...
```bash
nextflow pull nf-core/proteomicsTMT
```
...
First, go to the [nf-core/proteomicsTMT releases page](https://github.com/nf-core/proteomicsTMT/releases) and find the latest version number - numeric only (eg. `1.3.1`). Then specify this when running the pipeline with `-r` (one hyphen) - eg. `-r 1.3.1`.
...
* `docker`
  ...
  * Pulls software from Docker Hub: [`nfcore/proteomicsTMT`](https://hub.docker.com/r/nfcore/proteomicsTMT/)
* `singularity`
