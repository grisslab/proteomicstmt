...
3. Extracts and normalizes isobaric labeling information from an LC-MS/MS experiment.
4. Database search with either MSGF+ and/or Comet through OpenMS adapters
5. Re-mapping potentially identified peptides to the input database for consistency and error-checking (using OpenMS' PeptideIndexer)
6. PSM rescoring using PSMFeatureExtractor and Percolator or a PeptideProphet-like distribution fitting approach in OpenMS
7. If multiple search engines were chosen, the results are combined with OpenMS' ConsensusID
8. If multiple search engines were chosen, a combined FDR is calculated
9. Single run PSM/Peptide-level FDR filtering
10. If localization of modifications was requested, Luciphor2 is applied via the OpenMS adapter
11. Assigns protein/peptide identifications to features or consensus features.
12. Merges several files.
13. protein inference
14. Compute peptide and protein abundances from annotated feature/consensus maps or from identification results.

A rough visualization follows:
![proteomicsTMT workflow](./images/proteomicsTMT.svg)
...
* proteomics\_tmt
  * [protein\_out.csv](#protein-output)
  * [peptide_out.csv](#peptide-output)
  * [out.mzTab](#mztab)
...
### ProteomicsTMT main output

The `proteomics_tmt` folder contains the output of the pipeline without any statistical postprocessing.
...
If activated, the `ptxqc` folder will contain the report of the [PTXQC R package](https://cran.r-project.org/web/packages/PTXQC/index.html) based on the mzTab output of proteomicsTMT.
